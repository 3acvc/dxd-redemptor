...
describe("gnosis handler", () => {
    ...
    test("should ignore non-transfer event", async () => {
        ...
        const supply = await getDXDCirculatingSupplyAtBlocks(1);
    ...
    test("should reduce dxd supply on transfer to a 'holding' address", async () => {
        ...
        // dxd supply is checkpointed using mainnet blocks, let's mock the mainnet block returned
        // by the provider
        providerList[ChainId.ETHEREUM] = {
            getBlockNumber: jest.fn().mockReturnValueOnce(10),
        } as unknown as WebSocketProvider;
        await handle(providerList, {
            blockNumber: 200,
            blockHash: "",
