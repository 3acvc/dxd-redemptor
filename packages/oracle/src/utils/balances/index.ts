...
export async function getTokenBalancesAtBlockForProvider(
  ...
  const requestsByChain = requests.reduce(
    (accumulator: Record<ChainId, IndexedRequest[]>, { token, owner }, i) => {
  ...
  for (const [rawChainId, requestsInChain] of Object.entries(requestsByChain)) {
    ...
    for (let i = 0; i < resultsInChain.length; i++) {
      ...
      results[originalRequest.index] = Amount.fromRawAmount(
        ...
        ERC20_INTERFACE.decodeFunctionResult("balanceOf", resultInChain)[0]
      );
    }
  }
  return results;
