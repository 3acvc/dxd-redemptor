...
const handleToken = (token: Token): Token => {
  switch (token.chainId) {
    case ChainId.ETHEREUM:
      return token;
    case ChainId.GNOSIS: {
      if (token.equals(USDC[ChainId.GNOSIS])) return USDC[ChainId.ETHEREUM];
      if (token.equals(DXD[ChainId.GNOSIS])) return DXD[ChainId.ETHEREUM];
      if (token.equals(WXDAI)) return DAI;
      if (token.equals(WETH[ChainId.GNOSIS])) return WETH[ChainId.ETHEREUM];
      ...
      if (token.equals(WBTC[ChainId.GNOSIS])) return WBTC[ChainId.ETHEREUM];
      if (token.equals(SWPR[ChainId.GNOSIS])) return SWPR[ChainId.ETHEREUM];
      else return token;
    }
  }
};
...
> {
  ...
  const rawUSDValue = currencyAmounts.reduce(
    (usdValue: Decimal, currencyAmount) => {
      ...
      const price = new Amount(mainnetUsdc, formatUnits(_tokenPrice.usdPrice));
